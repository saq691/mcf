$(document).ready(function(){
	$('#dataList').on('change','.isContact',function(){
		$.ajax({
			type:'post',
			url:baseURL+'/contactInfo/isContact',
		...
		d=dialog({
			...
			ok:function(){
				...
				$.ajax({
					type:'post',
					url:baseURL+'/contactInfo/addRemark',
	...
	$('#delete').on('click',function(){
		...
		$.ajax({
			type:'post',
			url:baseURL+'/contactInfo/delete',
			data:{'ids':ids.join(",")},
			dataType:'json',
			success:function(data){
				var skin = data.status?'auto-close':'auto-warn';
	...
	$('#searchBtn').click(function(){
	    ...
	    if(flag){window.open(baseURL+'/contactInfo/list.html?'+$.param(lastParams),'_self');}
	});
